...
const ContactForm = (props: props) => {
  ...
  return (
    <>
      <dialog
        ref={props.modalRef}
        className="w-4/5 md:w-1/2 bg-white dark:bg-slate-800 rounded-md shadow-2xl dark:shadow-black fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 outline-none backdrop:bg-black/80"
      >
        <div className="flex justify-between text-slate-500 dark:text-blue-300  p-4">
          <h1 className="text-xl lg:text-2xl font-bold" style={poppins.style}>
            Get in touch
          </h1>
          <span
            onClick={() => props.modalRef.current?.close()}
            style={robotoMono.style}
            className="ring-1 rounded-md text-xs h-fit p-1 ring-slate-900/10 dark:ring-slate-700/50 shadow-sm ml-3 cursor-pointer"
          >
            Esc
          </span>
        </div>
    ...
    <>
      <div className="flex flex-col md:flex-row gap-4 w-full mb-4">
        <div className="w-full">
          <label
            htmlFor="name"
            className="block mb-2 text-base font-medium text-gray-600 dark:text-gray-300"
          >
            Your Name
          </label>
          <input
      ...
      <button
        type="submit"
        style={robotoMono.style}
        className="text-white bg-indigo-600 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-500 rounded-lg text-lg flex items-center w-fit ml-auto gap-2 disabled:cursor-none"
        disabled={pending}
      >
        {pending ? (
          <IoReload className="text-xl animate-spin" />
        ) : (
